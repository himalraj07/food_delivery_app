...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeCustomizableItem: (
      ...
      const restautantCart = state?.carts?.find(
        cart => cart?.restaurant?.id === restaurant_id,
      );

      if (!restautantCart) return;

      const item = restautantCart?.items?.find(
      ...
      if (customizationIndex !== -1 && item?.customizations) {
        ...
        if (item?.quantity === 0 || item?.customizations?.length === 0) {
          restautantCart.items = restautantCart?.items?.filter(
            cartItem => cartItem.id !== itemId,
          );
        }

        if (restautantCart?.items?.length === 0) {
          state.carts = state.carts?.filter(
            cart => cart?.restaurant?.id !== restaurant_id,
          );
        }
